...
const StickyHeader = ({ soundOn, onToggleSound, onLogout, onAdd, addGoalOpen, onAddGoalOpenChange }: StickyHeaderProps) => (
  <motion.div
    initial={{ y: -56, opacity: 0 }}
    animate={{ y: 0, opacity: 1 }}
    exit={{ y: -56, opacity: 0 }}
    transition={{ duration: 0.22, ease: 'easeOut' }}
    className="fixed top-0 left-0 right-0 z-40 border-b border-border/40 bg-background/75 backdrop-blur-xl"
  >
